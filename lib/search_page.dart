...
class _SearchPageState extends State<SearchPage> {
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (_doctorID != -1) {
                            ...
                            if (buildings.isNotEmpty) {
                              _buildingID = buildings[0].buildingID;
                            }
                          }
                      ...
                      StreamBuilder(
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (_departmentID != -1) {
                            doctors = doctors
                                .where((doctor) =>
                                    doctor.departmentID == _departmentID)
                                .toList();
                          }
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (_doctorID != -1) {
                            ...
                            if (departments.isNotEmpty) {
                              _departmentID = departments[0].departmentID;
                            }
                          }
  ...
  Future<void> _getDepartmentFromKeyword(String keyword) async {
    ...
    if (snapshot.size > 0) {
      ...
      final id = data['department_id'].toInt();
      setState(() {
        ///If the doctor dropdown is already selected, and it conflicts with the department from keyword, reset the doctor dropdown
        if (!snapshot.docs[0].data()['doctor_list'].contains(_doctorID)) {
          _doctorID = -1;
        }

        ///If the building dropdown is already selected, and it conflicts with the department from keyword, reset the building dropdown
        if (!snapshot.docs[0].data()['building_list'].contains(_buildingID)) {
          _buildingID = -1;
        }
        _departmentID = id;
      });
  ...
  /// Resets all values to -1 when reset button clicked.
  Future<void> _resetsAllValues() async {
    setState(() {
      _doctorID = -1;
      _buildingID = -1;
      _departmentID = -1;
